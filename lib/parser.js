...
class GitHubParser {
    ...
    extractWordsFromText(text, words) {
        ...
        // Split by various delimiters but preserve the original structure
        // Use a comprehensive regex to split on various delimiters while capturing the words
        const wordRegex = /[a-zA-Z][a-zA-Z0-9_]*(?:[a-zA-Z0-9_]*[a-zA-Z0-9])?/g;
        const matches = text.match(wordRegex) || [];

        matches.forEach(word => {
            word = word.trim();
            
            // Add the original word as-is if it's valid
            if (this.isValidWord(word)) {
                words.add(word); // Keep original case
                
                // Also add camelCase/PascalCase parts while preserving the original
                this.extractCamelCaseWords(word, words);
            }
        });

        // Also extract words by splitting on common separators
        const separatedWords = text.split(/[\s\(\)\[\]{};,.<>!@#$%^&*+=|\\`~"'/:?-]+/);
    ...
    extractWordsFromFileName(fileName, words) {
        ...
        // Also extract individual path components
        const pathParts = fileName.split(/[\/\\]/);
        pathParts.forEach(part => {
            if (part && this.isValidWord(part)) {
                words.add(part);
                this.extractCamelCaseWords(part, words);
            }
        });
    }
    ...
    extractCamelCaseWords(word, words) {
        ...
        camelCaseParts.forEach(part => {
            // Keep the original case of each part
            if (this.isValidWord(part)) {
                words.add(part); // Preserve original case
            }
        });

        // Also handle snake_case and kebab-case
        const underscoreParts = word.split('_').filter(part => part.length > 0);
        ...
        const dashParts = word.split('-').filter(part => part.length > 0);
